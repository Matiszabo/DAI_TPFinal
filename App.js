...
function ScreenA2({ route }) {
    ...
    const fetchParticipants = async () => {
      try {
        const response = await fetch(
          'https://pleased-vocal-corgi.ngrok-free.app/api/event/enrollments/' + item.id
        );
  ...
  const Desuscribirse = async(id)=> {
    try {
      const response = await fetch("https://pleased-vocal-corgi.ngrok-free.app/api/event/" + id+"/enrollment", { 
      method: 'DELETE',
      headers: {
  ...
  useEffect(() => {
    const prepareApp = async () => {
      try {
        ...
        const response = await fetch('https://pleased-vocal-corgi.ngrok-free.app/api/event');
        const data = await response.json();
